...
**2026-09-26、接続3回目で確認済み(成功)。** `CHGJOB INQMSGRPY(*DFT)` は
RPG0102 を `C`(取消)で自動応答し、ハングを防ぐことが実機で確定した。
詳細は `docs/probes.md` の「検証ハーネス(`verify/`)自身の実機確認」節を
参照。以下は接続1〜3回目を通して使った判定基準(今後、このマニフェストを
再実行する機会があれば同じ基準で読むこと)。

このバッチは元々「`CHGJOB INQMSGRPY(*DFT)` が RPG0102(ゼロ除算の照会
メッセージ)を自動応答してハングを防げるか」という、実機で確認していない
問いそのものを検証する目的で作った。したがって単一の「正解の出力」を
事前に決め打ちできない(それを決めるのがこの接続の目的だった)。両方の
結果を想定し、どちらが起きても誤判定しないよう次のとおり記録していた。
