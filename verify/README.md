...
- `file` ステップ: `localPath`(このマニフェストのディレクトリーからの相対パス)の中身を heredoc で転送し、`CPYFRMSTMF` で `library` の `remoteSrcFile`/`member` に取り込む。`ccsid` を**省略すると既定値1208が使われ**、`setccsid`/`STMFCCSID()` が出る(下記「既知の未検証事項」参照)。`false` を明示した場合だけ、これらを一切出さない。`remotePath`(省略可、既定は `<remoteDir>/<localPathのbasename>`というフラットな置き方)を指定すると、`$HOME`からの任意の相対パスへ直接置ける(2026-09-27追加)。`remoteSrcFile`/`member`も省略すると、IFS上にファイルを置くだけで終わり、`CPYFRMSTMF`は一切実行されない——`TXLEGACY`/`TXSNAP`/`TXMIGR`のように`CLONEDIR`配下の実ツリーを前提に**自分自身で**`CPYFRMSTMF`するツールを検証するとき、そのツールが期待する相対パスへ直接ファイルを置くのに使う(`verify/part05-txlegacy-exec`参照)。
