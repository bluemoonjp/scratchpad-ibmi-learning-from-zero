...
1. 長めのタイムアウト(数分)でジョブが実は完走しているだけなのかを確認する(TXSETUP のハングのときと同様、SSH 側は切れてもサーバー側でジョブが継続し、後から結果が取れる可能性がある)。
2. `F`(全手続き方式)ではなく、`I`(通常の入力・O仕様書なしでも動く一次資料ベースの最小構成)や、あるいは意図的にキーなしの物理ファイルで同様の最小テストを行い、`TOKUIM` 固有の問題(キー定義)かどうかを切り分ける。
3. 05250 の対話的セッションで同じプログラムを実行し、SSH 経由の `system()` に固有の問題かどうかを確認する。
