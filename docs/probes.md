...
**未完了**: `<USER>2/TXSETUP LIB(<USER>2)` のように**ライブラリー修飾したコマンド名**で実行を試みた直後に、SSH(2222番)が再び接続数制限に触れて `Connection timed out` になった(992番・443番は生きている、これまでと同じパターン)。**コマンド定義の作成(コンパイル)は確認済みだが、実際にコマンドとして呼び出して `TXSETUP` が完走するかどうかの確認は次回に持ち越し。** 次回最優先で確認し、確認でき次第 02-05 のレッスン本文を「`CALL PGM(TXSETUP)`」から「`TXSETUP`(コマンドとして直接入力)」に書き換える。

**影響**: tools/qcmdsrc/(新規、コンパイル確認済み・実行未確認)。02-05(次回、コマンド経由の呼び出しに書き換え予定)。
