...
## 未解決: DISK ファイルへの READ が実行時にハングする(確認日 2026-09-25)
...
1. **5250 の対話的セッションで同じ最小プログラムを実行し、SSH 経由の `system()` に固有の問題かどうかを確認する。** これが最も有力な仮説(QUSER問題と同様、PASE の `system()` 経由のジョブに特有の制約がある可能性)。
2. `F`(全手続き方式)ではなく `P`(プライマリー・ファイル、RPG サイクルによる自動読み込み)の最小テストを**コンパイルまでは実施済み**(0エラー)。**実行(`CALL`)を試みた直後に SSH が接続数制限に触れて `Connection timed out` になり、`P` 指定でも同じハングが起きるかどうかはまだ確認できていない。** 次回最優先。
3. `TOKUIM` 以外の物理ファイル(例えば `TANTOM`)でも同じハングが起きるか確認し、`TOKUIM` 固有(キー定義・レコード長・CCSID)の問題かどうかを切り分ける。
4. IBM の一次資料で、PASE の `system()` 経由のジョブから DB ファイルを開く際の既知の制約がないか確認する。

**影響**: 04-06(外部記述ファイルを順に読む)は、READ命令の桁位置は確定したが、**実際にファイルを読む通し検証がまだ完了していない。** レッスン本文は桁位置が確定した範囲で書き、実機メモで「実行時のハングを調査中」と明記する。
