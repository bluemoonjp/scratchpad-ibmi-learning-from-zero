...
**実行そのものの検証は継続中。** `CALL PGM(AOTSUKI2/TXSETUP) PARM(...)` を投入したところ、想定より長く応答が返っていない(6物理ファイル+2論理ファイルの作成と初期データ約30行の `RUNSQLSTM` を1呼び出しで行うため、多少時間がかかる可能性はあるが、通常はここまで長くはかからないはずである)。結果が判明次第、この節を更新する。
