...
- 確認日: 2026-09-25。金額の計算(`5560.00`・`9170.00`)は、`db/data/load_v1.sql` の実データから著者が手計算で検証済み。**`TXSETUP` は実機での完走を確認済み**(02-05 参照)で、このデータが実際にデータベースへ投入されることも SQL の件数確認で検証済み。ただし、このレッスンで示す `SELECT` 文そのものを `STRSQL` で実行して画面表示を確認するところまでは、このセッションではまだ行っていない。次回確認でき次第、この表記を更新する。
