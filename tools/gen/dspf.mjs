...
// 条件標識(フィールド・定数行を標識でON/OFFする書き方、桁7-16)は、IBM の DDS の
// 一般公開資料で長年安定している桁位置(桁7=関係(A/O、同じフィールド/定数を追加の
// 標識条件で継続する2行目以降に指定)、桁8-16=標識3組×3桁(各組「N(否定、任意)+
// 標識2桁」))を採用しているが、**このリポジトリーではまだ実機コンパイルで確認して
// いない**(04-11 で保留した点、実行計画のプローブ「5」で確認する予定)。使う前に
// 必ず候補どおりコンパイルが通ることを実機で確かめること。
